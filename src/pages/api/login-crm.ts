...
const appJwtEnpoint = `${process.env.NEXT_PUBLIC_DEFAULT_PROXY}/${process.env.NEXT_PUBLIC_APP_API_MSCSM}/api/authorization/gen-app-jwt`;
  ...
  try {
    ...
    if (responseAuth.status === 200) {
      ...
      if (responseGetToken.status === 200) {
        const authToken = responseGetToken.data.id_token;
